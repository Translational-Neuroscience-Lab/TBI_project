...
% Tests whether Control vs Trazodone differences change across sessions

% Add PLS toolbox to path
addpath(genpath('PLS'));

% Parameters
sessions = {'Session1', 'Session2', 'Session4','Session5', 'Session6', 'Session7'};
conditions = {'TBI', 'Sham'};
...
expected_n = struct('Control', 5, 'Trazodone', 8);
                            ...
                            for stage_idx = 1:length(unique_stages)
                                ...
                                for light_idx = 1:length(unique_lighting)
                                    ...
                                    if sum(combo_idx) >= 3 % Minimum 3 epochs
                                        ...
                                        mouse_data.mouse_ids{end+1} = sprintf('%s_%s_%s', mouse_id, stage, lighting);
                                        mouse_data.stages = [mouse_data.stages; stage];
                                        mouse_data.lighting = [mouse_data.lighting; lighting];
...
for ch = 1:length(channels)
    ...
    for st = 1:length(stages)
        ...
        for lc = 1:length(lighting_conditions)
            ...
            for s = 1:length(sessions)
                session = sessions{s};
                session_num = str2double(regexp(session, '\d+', 'match', 'once'));
                    ...
                    if isfield(all_data, session) && isfield(all_data.(session), condition) && ...
                        ...
                        if ~isempty(data_struct.mouse_averages)
                            ...
                            if sum(combined_idx) > 0
                                ...
                                group_code = strcmp(condition, 'Trazodone');
                                group_labels = [group_labels; repmat(group_code, size(condition_data, 1), 1)];
                                session_labels = [session_labels; repmat(session_num, size(condition_data, 1), 1)];
                            end
                        end
                    end
                end
            ...
            if ~isempty(interaction_data)
                ...
                % All Controls first, then all Trazodone
                for s = 1:length(unique_sessions)
                    session_num = unique_sessions(s);
                    
                    % Control
                    session_idx = session_labels == session_num;
                    group_idx = group_labels == 0; % Control = 0
                    ...
                    if sum(combo_idx) >= 3
                        ...
                        group_labels_pls{end+1} = sprintf('Control S%d', session_num);
                    end
                end
                ...
                for s = 1:length(unique_sessions)
                    ...
                    % Trazodone
                    session_idx = session_labels == session_num;
                    group_idx = group_labels == 1; % Trazodone = 1
                    combo_idx = session_idx & group_idx;
                    ...
                    if sum(combo_idx) >= 3
                        ...
                        pls_data = [pls_data; combo_data];
                        group_sizes(end+1) = size(combo_data, 1);
                        group_labels_pls{end+1} = sprintf('Trazodone S%d', session_num);
                    end
                end
                ...
                if length(group_sizes) >= 2 && min(group_sizes) >= 3
                    ...
                    if isfield(pls_result, 'perm_result') && isfield(pls_result.perm_result, 'sprob')
                        ...
                        if length(pvals) >= 1
                            LV = 1;
                            p = pvals(LV);
                            headline = sprintf('Group×Session: %s %s Ch%d, LV%d p=%.4f', ...
                            ...
                            for i = 1:length(group_labels_pls)
                                if contains(group_labels_pls{i}, 'Control')
                            ...
                            % Separator line between Control and Trazodone groups
                            num_controls = sum(contains(group_labels_pls, 'Control'));
                            xline(num_controls + 0.5, '--k', 'LineWidth', 2, 'Alpha', 0.7);
                            ...
                            % Save figure
                            fig_filename = fullfile(results_dir, sprintf('GroupSession_PLS_%s_%s_ch%d_LV%d.png', ...
                                stage, lighting, channel, LV));
                            saveas(gcf, fig_filename)
                            
                            fig_filename_highres = fullfile(results_dir, sprintf('GroupSession_PLS_%s_%s_ch%d_LV%d.fig', ...
                                stage, lighting, channel, LV));
                        ...
                        % Save results
                        save_filename = fullfile(results_dir, sprintf('GroupSession_PLS_%s_%s_ch%d.mat', ...
                            stage, lighting, channel));
                        save(save_filename, 'pls_result', 'pvals', 'group_sizes', 'conditions', ...
                            'sessions', 'stage', 'lighting', 'channel', 'interaction_data', ...
