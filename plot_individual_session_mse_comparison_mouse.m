...
% Single session analysis without lighting condition separation

sessions = {session};  % Single session
conditions = {'TBI', 'Sham'};
...
fprintf('Loading data from session for both channels...\n');
...
for ch = 1:length(channels)
    ...
    for s = 1:length(sessions)
        ...
        for c = 1:length(conditions)
            ...
            condition_path = fullfile(base_path, current_session, condition, 'Results');
            ...
            if exist(condition_path, 'dir')
                ...
                if ~isempty(dataset_files)
                    ...
                    session_condition_data.mse_data = [];
                    session_condition_data.stages = [];
                    session_condition_data.scales = [];
                    session_condition_data.subject_ids = [];
                    session_condition_data.individual_data = {};

                    % Load and combine all subjects for this session/condition
                    for f = 1:length(dataset_files)
                        ...
                        try
                            ...
                            if ~isempty(valid_mse)
                                ...
                                session_condition_data.individual_data{end+1} = subject_data;
                                
                                % Concatenate data for overall statistics
                                session_condition_data.mse_data = [session_condition_data.mse_data; mse_matrix];
                                session_condition_data.stages = [session_condition_data.stages; valid_stages];
                                session_condition_data.scales = dataset.mse_scales;
                                
                                % Add subject IDs for each epoch
                                subject_ids = repmat(f, n_valid, 1);
                                session_condition_data.subject_ids = [session_condition_data.subject_ids; subject_ids];
                            end
                    ...
                    all_data.(current_session).(condition).(field_name) = session_condition_data;
                    fprintf('    %s Channel %d: %d epochs from %d subjects loaded\n', condition, channel, ...
                           size(session_condition_data.mse_data, 1), length(session_condition_data.individual_data));
...
fprintf('\nCreating session plots...\n');

for ch = 1:length(channels)
    channel = channels(ch);
    fprintf('Processing Channel %d plots...\n', channel);

    % Session comparison plots for this channel
    for s = 1:length(sessions)
        current_session = sessions{s};
        fprintf('  Creating plots for %s...\n', current_session);

        % Create mse_plots directory for this session
        plot_dir = fullfile(base_path, current_session, 'mse_plots');
        ...
        for st = 1:length(stages)
            ...
            for c = 1:length(conditions)
                ...
                if isfield(all_data, current_session) && isfield(all_data.(current_session), condition) && ...
                   isfield(all_data.(current_session).(condition), field_name)

                    data_struct = all_data.(current_session).(condition).(field_name);
            ...
            if ~isempty(legend_handles)
                ...
                title(sprintf('%s: %s Stage - Channel %d', current_session, stage, channel), 'FontSize', 14);
                legend(legend_handles, legend_entries, 'Location', 'best', 'FontSize', 10);
                grid on;

                % Save plot
                plot_filename_png = fullfile(plot_dir, sprintf('%s_%s_ch%d_individual.png', ...
                                           current_session, stage, channel));
                plot_filename_fig = fullfile(plot_dir, sprintf('%s_%s_ch%d_individual.fig', ...
                                           current_session, stage, channel));
